  ...
  return (
    <Card className="border-sky-700 bg-sky-800/80 shadow-lg">
      ...
      <CardContent className="pt-0">
        {foundWords.length === 0 ? (
          <p className="text-sky-300 text-center py-2 text-sm">No words found yet</p>
        ) : (
          <div className="flex flex-wrap gap-2 max-h-[150px] overflow-y-auto pr-1">
            {foundWords.map((word, index) => (
              <span
                key={index}
                className="bg-sky-900/80 text-amber-300 px-2 py-1 rounded-md text-xs border border-sky-700"
              >
                {word}
              </span>
            ))}
